...
//           <button className="w-full bg-blue-600 text-white py-2 rounded-lg hover:cursor-pointer hover:bg-blue-700">
//             Apply Filters
//           </button>
//         </div>
...
import { setSearchFilters } from "../Features/searchslice"; // ✅ Correct
import Footer from "../components/Footer";
import Hostels from "../components/Hostels";
...
function Listing() {
  ...
  const [selectedHostels, setSelectedHostels] = useState([]);
  const [isCompareOpen, setIsCompareOpen] = useState(false);
  const [searchFilters, setSearchFiltersState] = useState({
    institution: "",
    radius: "",
    location: "",
    amenities: [],
      ...
      <div className="flex flex-col lg:flex-row lg:justify-center items-center justify-center mt-7 gap-6 px-4 lg:px-8">
        <div className="w-[300px] bg-white p-4 rounded-lg shadow-lg">
          ...
          <div className="mb-4">
            ...
            />
            <p className="text-sm text-gray-600 mt-1">Max Price: {searchFilters.maxPrice}</p>
          </div>

          <div className="mb-4">
            <h3 className="text-sm font-medium text-gray-700">Amenities</h3>
            <div className="flex flex-col space-y-1 text-sm text-gray-600">
              {['Air Conditioning', 'Free Wi-Fi', 'Kitchen Facilities', 'Parking'].map((item) => (
                <label key={item}>
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={searchFilters.amenities.includes(item)}
          ...
          <button
            className="w-full bg-blue-600 text-white py-2 rounded-lg hover:cursor-pointer hover:bg-blue-700"
            onClick={handleApplyFilters}
          >
            Apply Filters
          </button>
